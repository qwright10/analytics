...
export default class ChannelCommand extends Command {
    ...
    public async exec(message: Message, { channel }: { channel: Channel }): Promise<Message | Message[]> {
        ...
        const channelRecord: any = await getRepository(CEntity).findOne({ id }) || this.client.channels.cache.get(id);
        ...
        if (channel instanceof CEntity) {
            const embed = new MessageEmbed()
                ...
                .addField('Name (ID)', `${channelRecord.name} (${channelRecord.id})`)
            ...
            embed.addField('Topic', channelRecord.topic || 'None')
